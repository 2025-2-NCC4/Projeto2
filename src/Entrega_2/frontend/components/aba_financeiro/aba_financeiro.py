...
def render_aba_financeiro(df_filtrado: pd.DataFrame, lojas=None, categorias=None, export: bool = False):
    """
    Aba Financeiro. Se export=True, retorna {titulo: fig}.
    """
    ...
    f = _col_receita_custo_lucro_semana(dff)
    if f: figs["Receita × Custo × Lucro por Semana"] = f
    f = _col_comissao_picmoney_semana(dff)
    if f: figs["Comissão PicMoney Média por Semana (%)"] = f
    f = _lin_receita_custo_lucro_diario(dff)
    if f: figs["Receita × Custo × Lucro por Dia"] = f
    f = _lin_ticket_medio_diario(dff)
    if f: figs["Ticket Médio por Dia (R$)"] = f
    f = _lin_transacoes_diario(dff)
    if f: figs["Transações por Dia"] = f
    f = _col_margem_media_por_loja(dff, top=15, min_receita=10000)
    if f: figs["Margem Média por Loja (%)"] = f
    f = _col_participacao_lucro_por_categoria(dff, min_lucro=0)
    if f: figs["Participação no Lucro Bruto por Categoria (%)"] = f
    f = _col_receita_dia_semana(dff)
    if f: figs["Receita por Dia da Semana (R$)"] = f
